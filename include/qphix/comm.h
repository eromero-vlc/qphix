...
class Comms
{
 public:
    ...
    for (int d = 0; d < 4; d++) {
      if (!localDir(d)) {
        ...
        if (bw_neigh_coords[d] < 0)
          bw_neigh_coords[d] = qmp_dims[d] - 1;
        fw_neigh_coords[d]++;
        if (fw_neigh_coords[d] == qmp_dims[d])
          fw_neigh_coords[d] = 0;
  ...
  {
#ifdef QMP_DIAGNOSTICS
    printf("My Rank: %d, finish recv from dir %d,  My Records: srce=%d, dest=%d "
           "len=%d\n",
           myRank,
           d,
           myNeighboursInDir[d],
