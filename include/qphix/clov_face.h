    ...
    bool const is_plus)
{
  ...
  for (int pkt = low_pkt + smtid; pkt < high_pkt; pkt += n_threads_per_core) {
    ...
    int pkt_next = pkt + n_threads_per_core < high_pkt ? pkt + n_threads_per_core
                                                       : low_pkt + smtid;
    ...
    if (dir == 0 && ngy == 1)
      yi_next += (fb == 0 ? 1 - xodd_next : xodd_next);
    ...
    int off_next = (t_next - t) * Pxyz + (z_next - z) * Pxy +
                   (yi_next - yi) * n_soa_x + (xblock_next - xblock);
    ...
    bool const is_plus)
{
  ...
  for (int pkt = low_pkt + smtid; pkt < high_pkt; pkt += n_threads_per_core) {
    ...
    int xodd_next = (z_next + t_next + cb) & 1;
    if (dir == 0 && ngy == 1)
      yi_next += (fb == 0 ? xodd_next : 1 - xodd_next);
    // printf("Pack %d %d %d %d\n", t, z, yi, xblock*soalen);
    ...
    int hsprefdist = (pkt_next - pkt) * sizeof(FourSpinorBlock) / sizeof(FT) / 2;
    int goff_next =
        ((t_next - t) * Pxyz + (z_next - z) * Pxy + (yi_next - yi) * n_soa_x) / ngy +
        (xblock_next - xblock);
    ...
    const CloverBlock *clBase =
        &invclov[(t * Pxyz + z * Pxy + yi * n_soa_x) / ngy + xblock];
    const int clov_line_in_floats =
        sizeof(CloverBlock) / sizeof(FT); // One gauge scanline, in floats
